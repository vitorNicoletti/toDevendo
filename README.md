...
└── app/
    ...
    └── src/
        ...
        ├── lid_resolver.py  # Resolve IDs do WhatsApp no telefone canônico
...
- **Usuario** — membros cadastrados, identificados pelo `telefone` (número puro).
- **Divida** — dívidas entre usuários (devedor, credor, valor, status).
- **UsuarioLid** — vincula os IDs opacos `@lid` do WhatsApp a um `Usuario`
  (um usuário pode ter vários lids).

O WhatsApp identifica uma pessoa ora pelo número (`...@c.us`/`...@s.whatsapp.net`),
ora por um ID opaco (`...@lid`). O bot reduz tudo ao número puro: JIDs de
telefone viram dígitos diretamente; `@lid` é traduzido consultando os
participantes do grupo na WAHA e o resultado fica em cache na tabela `UsuarioLid`.
