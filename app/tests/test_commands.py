...
ANA = "111111111111@lid"
BRUNO = "222222222222@lid"
...
def test_cadastro_cria_usuario(db):
    ...
    assert uc.get_by_jid(db, ANA).nome == "Ana Maria"


def test_cadastro_existente_atualiza_nome(db):
    _disp("!cadastro Ana", remetente=ANA, db=db)
    resposta = _disp("!cadastro Ana Silva", remetente=ANA, db=db)
    assert "atualizado" in resposta.lower() or "Ana Silva" in resposta
    assert uc.get_by_jid(db, ANA).nome == "Ana Silva"
...
def test_pix_de_mencionado_inexistente(db):
    _disp("!cadastro Ana", remetente=ANA, db=db)
    resposta = _disp("!pix @x", remetente=ANA, mencionados=["999@lid"], db=db)
